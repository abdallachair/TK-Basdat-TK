$(document).ready(function(){
    ...
    $("#addJasaKirim").click(function () {
        ...
        var app = "<?php";
    $("#jasaKirim").append(
        '<br><label for="harga">Jasa Kirim '+nomor+'</label>' + '<select name="jasa_kirim_' + nomor + '">' + app+ '$jasa = selectAllFromTable(' + '"TOKOKEREN.JASA_KIRIM");' +'while($row = pg_fetch_row($jasa)){'+ "echo '" +'<option value="' +".$row['0']."+'">'+".$row['0']."+"</option>';}"+ "$_SESSION['no_jasa_kirim'] ="+ nomor +";?></select>");
 });
})
